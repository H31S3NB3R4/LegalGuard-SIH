...
PURPOSE: Primary product-compliance analysis + general-compliance chatbot
(JSON/text based, via LangChain + Google Gemini 2.5 Flash).

Used by these live routes in server.py:
  - POST /api/compliance/analyze/<int:product_id>   -> compliance.analyze_compliance()
  - POST /api/compliance/batch                      -> compliance.batch_analyze_products()
  - POST /api/chat                                  -> compliance.chatbot_agent() (general-compliance intent)

Distinct sibling modules (do NOT consolidate — each is used by different routes):
  - compliance_copy.py: duplicate analyze_compliance() variant + text-based seller-upload
    check. Used by /api/scrape auto-analysis, /api/products/validate/<id>,
    and /api/seller/check-upload-text.
...
import ai_guard
from grounding import retrieve_rules, format_context as _grounding_format_context
...
if AI_AVAILABLE:
    ...
    llm = ChatGoogleGenerativeAI(
        ...
        # Fail fast: raise 429 immediately instead of SDK-long backoff retries.
        max_retries=0
    )
    multimodal_model = genai.GenerativeModel('gemini-2.5-flash')
else:
    print("[WARN] GOOGLE_API_KEY not set — LLM disabled for compliance.py; server will run in demo mode.")


def get_llm():
    """Return the chat model (Phase 12).

    Previously analyze_compliance's assessment step called `get_llm()` — a
    name this module never defined — so EVERY run crashed with
    NameError inside the try/except and silently used the deterministic
    assessment. The accessor now simply returns the module-level model
    (built at import when a key exists, else None).
    """
    return llm
...
"""
    ...
    if llm is None or not ai_guard.ai_ok():
        log("[DATA ANALYSIS] Skipping Gemini (no key or rate-limit cooldown) "
            "-> deterministic offline extraction")
        return _offline.offline_data_findings(
            product_data, category, all_rules
        )
...
"""
    ...
    if not ai_guard.ai_ok():
        return ("major", -4.0) if is_partial else ("major", -10.0)
...
def calculate_compliance_score(
    ...
    valid_data_findings = _valid_citation_findings(
        data_analysis.get("findings", []), all_rules, log
    )
    ...
    # Phase 12: surface degraded mode (listing data came from the
    # deterministic offline extractor rather than Gemini).
...
def analyze_compliance(product_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        compliance_report = {
            ...
            'requires_action': (
                _indeterminate or
                len([v for v in scoring['violations'] if v.get('severity') == 'critical']) > 0
            ),
            'log':LOG_BUFFER
        }
    ...
    try:
        ...
        feedback_report = {
            ...
            'ready_for_upload': bool(
                scoring.get('score') is not None and scoring['score'] >= 70
                and len([v for v in scoring['violations'] if v.get('severity') == 'critical']) == 0
            ),
            'estimated_approval_chance': (
                'High' if (scoring.get('score') or 0) >= 85 else
                'Medium' if (scoring.get('score') or 0) >= 70 else 'Low'
            )
        }
        
        print(f"[SELLER UPLOAD CHECK] SUCCESS: Score: {scoring.get('score')}/100 | Grade: {scoring['grade']}")
        ...
        import traceback
        traceback.print_exc()
        # Phase 12: an AI/analysis outage is NOT a product failure — never a fake 'F'.
        return {
...
def _offline_grounded_answer(user_message: str) -> str:
    ...
    return ("Gemini is temporarily rate-limited, so here is an offline answer from the "
            "Legal Metrology rule base:\n\n" + _grounding_format_context(units))
...
def chatbot_agent(user_message: str, conversation_history: List[Dict] = None) -> str:
    ...
    try:
        ...
        # Phase 11 / TODO #6: ANY Gemini failure (503 overload, timeout, network)
        # still gets a substantive offline rule-based answer instead of a
        # useless apology — the app stays usable when Gemini is unavailable.
        offline = _offline_grounded_answer(user_message)
        if offline.startswith("Gemini is temporarily rate-limited"):
            offline = offline.replace(
                "Gemini is temporarily rate-limited, so here is an offline answer",
                "The AI model is temporarily unavailable, so here is an offline answer",
                1,
            )
        return offline
