...
# Phase 9 RAG grounding (stdlib-only, offline).
from grounding import grounding_context_for, format_context
import ai_guard
...
# before Flask ever bound its port. The model is therefore built on FIRST USE
# in get_llm(), inside a daemon worker thread with a short timeout; on
...
if not AI_AVAILABLE:
    print("[WARN] GOOGLE_API_KEY not set — LLM disabled for chatbot_compliance; server will run in demo mode.")
...
def get_llm():
    ...
    with _llm_init_lock:
        ...
        worker = threading.Thread(target=_worker, daemon=True, name="llm-init-chatbot")
        worker.start()
        worker.join(_LLM_INIT_TIMEOUT)
        if "llm" in box:
            llm = box["llm"]
            print("[OK] Gemini chat model initialised on first use (chatbot_compliance).")
        else:
            ...
            print(f"[WARN] Gemini chat model init failed: {reason} — "
                  "chatbot_compliance runs in demo mode until restart.")
    return llm
...
def create_user_aware_agent(user_id: int):
    ...
    try:
        model = get_llm()
        if model is None:
            print("[ERROR] LLM unavailable (demo mode) — cannot create user-aware agent")
            return None, None
        ...
        db_url = f"mysql+mysqlconnector://{DB_CONFIG['user']}:{password}@{DB_CONFIG['host']}:{DB_CONFIG['port']}/{DB_CONFIG['database']}"
...
def _offline_grounded_answer(user_message: str) -> str:
    ...
    units = grounding_context_for(user_message, top_k=6)
    if not units:
...
def user_chatbot(user_id: int, user_message: str, conversation_history: List[Dict] = None) -> Dict[str, Any]:
    ...
    if not ai_guard.ai_ok():
        return {
            'response': _offline_grounded_answer(user_message),
            'user_context': None,
...
Remember: Only show data for User ID {user_id}. Always filter with WHERE user_id = {user_id} for Products table."""
        ...
        # Get response from agent; ground the answer in the Legal Metrology rules (Phase 9).
        units = grounding_context_for(user_message, top_k=6)
        grounded_input = enhanced_message + (\
            "\n\nLegal Metrology GROUNDING CONTEXT (cross-check every answer against these rules "
            "and cite their IDs [LM-...]; never invent rule numbers):\n" +
            format_context(units)
        )
        result = agent.invoke({"input": grounded_input})
