...
> **Canonical entry point:** `server.py`. (No archived prototypes remain; the
> legacy `app.py`/`main.py`/`rag_compliance.py` files were deleted.)
...
### 5. (Optional, recommended for demos) Seed Demo Data
```bash
# from this directory (legal_metrology_backend/legal_metrology)
.\venv\Scripts\python.exe seed_demo_data.py     # Windows
python3 seed_demo_data.py                       # macOS / Linux
```

The seed is **idempotent** — running it twice is a no-op. It creates:

* a `demo` user — login `demo` / `Dem0@LegalGuard!` (customer role, 120 Meta-Tokens),
* 3 realistic products with **persisted compliance reports, remarks and
  assessments**: one fully compliant (`A+`), one partially compliant (`C`),
  one non-compliant (`F`) — so Products, grade filters and the dashboard have
  data out of the box,
* seller locations on each product so the global heatmap renders 3 points,
* 3 gift/reward catalogue entries.
